...
const showTemplate = () => {
  confirm.require({
    group: "templating",
    header: t("common.settings.delete-confirm-header"),
    message: t("common.settings.delete-confirm-message"),
    rejectProps: {
      label: t("common.settings.delete-confirm-cancel"),
      icon: "pi pi-times",
      outlined: true,
      size: "small",
    },
    acceptProps: {
      label: t("common.settings.delete-confirm-delete"),
      icon: "pi pi-check",
      size: "small",
    },
    accept: () => {
  ...
  <section class="settings-section">
    <div class="photo-row row">
      <div class="label-wrapper">
        <label>{{ t("common.settings.profile-photo") }}</label>
        <span class="label-hint">
          {{ t("common.settings.photo-hint") }}
        </span>
      </div>
    ...
    >
      <div class="form-row row">
        <div class="label-wrapper">
          <label for="username">{{ t("common.settings.name") }}</label>
          <span class="label-hint">
            {{ t("common.settings.name-hint") }}
          </span>
        </div>
        <div class="input-wrapper">
          ...
          <Button
            ...
            type="submit"
            :label="t('common.settings.change-name')"
            class="submit-button"
          />
        </div>
      </div>
    ...
    >
      <div class="form-row">
        <div class="label-wrapper">
          <label for="username">{{ t("common.settings.email") }}</label>
          <span class="label-hint">
            {{ t("common.settings.email-hint") }}
          </span>
        </div>
        <div class="input-wrapper">
          <InputText
          ...
          <Button
            ...
            type="submit"
            :label="t('common.settings.change-email')"
            class="submit-button"
          />
        </div>
      </div>
    ...
    <div style="display: flex; gap: 15px; width: 100%; justify-content: flex-start">
      <Button
        :label="t('common.settings.change-password')"
        class="delete-button"
        @click="editPasswordVisible = !editPasswordVisible"
      />
      <Button
        :label="t('common.settings.delete-account')"
        class="delete-button"
        @click="showTemplate()"
      />
      ...
      <Dialog
        ...
        modal
        :header="t('common.settings.change-password')"
      >
